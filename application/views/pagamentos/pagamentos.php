<?php if ($this->permission->checkPermission($this->session->userdata('permissao'), 'aPagamento')) { ?>
    <a href="<?php echo base_url(); ?>pagamentos/adicionar" class="btn btn-success"><i class="fas fa-plus"></i> Adicionar Credencial de Pagamento</a>
<?php } ?>
                ...
                <?php 
                    ...
                    foreach ($results as $r) {
                        ...
                        if ($this->permission->checkPermission($this->session->userdata('permissao'), 'vPagamento')) {
                            echo '<a style="margin-right: 1%" href="' . base_url() . 'pagamentos/visualizar/' . $r->idPag . '" class="btn tip-top" title="Ver mais detalhes"><i class="fas fa-eye"></i></a>';
                        }
                        if ($this->permission->checkPermission($this->session->userdata('permissao'), 'ePagamento')) {
                            echo '<a style="margin-right: 1%" href="' . base_url() . 'pagamentos/editar/' . $r->idPag . '" class="btn btn-info tip-top" title="Editar"><i class="fas fa-edit"></i></a>';
                        }
...
<div id="modal-excluir" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <form action="<?php echo base_url() ?>pagamentos/excluir" method="post">
